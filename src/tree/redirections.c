...
void init_delimiters(t_ast_node *node, t_heredoc *heredoc)
{
	...
	i = 0;
	j = 0;
    heredoc->delimiter = malloc(sizeof(char *) * (node->redirection->heredoc_count + 1));
...
void handle_heredoc_input_helper(t_heredoc *heredoc, t_ast_node *node, t_minishell *data)
{
	char *tmp;


	if (ft_strcmp(heredoc->line, heredoc->delimiter[heredoc->current_delimiter]) == 0)
		(heredoc->current_delimiter)++;
	else if (heredoc->current_delimiter == node->redirection->heredoc_count - 1 && node->command[0])
	{
		tmp = heredoc->line;
...
void handle_heredoc_input(t_minishell *data, t_ast_node *node, t_heredoc *heredoc)
{
    while (1)
    {
		...
		if (!(heredoc->line))
		{
			if (heredoc->current_delimiter < node->redirection->heredoc_count - 1)
			{
				(heredoc->current_delimiter)++;
				continue;
			}
